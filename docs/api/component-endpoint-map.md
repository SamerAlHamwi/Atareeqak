...
| Customer name / photo / account-status badge | `components/ChatThread.tsx`, `components/ConversationList.tsx` | ✅ from the `user` block, once [BUG-14](./backend-issues.md) was fixed — it was null on every support conversation before that. `hooks/useChat.ts` keeps its reconstruction from message senders as a fallback for a backend without the fix |
