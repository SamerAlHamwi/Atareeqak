...
| [`verify-shell.mjs`](./verify-shell.mjs) | Phase 12/15 acceptance check — the app shell: `/` redirects by role, `/settings` no longer resolves, the header identity dropdown's name/role/email/`last_login_at` match `GET /staff/me` exactly, the language toggle flips `dir` inside the shell (labels read from the locale JSON, not hardcoded), the logout replay (capture a token, log out, replay it, assert `401`), no request to any removed route during a full walk of every page, and no raw i18n key leaks — all in both languages | 46 assertions, 0 failures |
...
| [`seed-phase-chat.sql`](./seed-phase-chat.sql) | The deliberate temporary seed the Chat page needs — `conversations` and `messages` both ship empty. 14 conversations / 87 messages covering two list pages, two message pages, both message types, an empty thread, both `sent_by_agent` directions, and one `private` control row that exposes [BUG-14](./backend-issues.md). **Its agent is `users.id = 36`**, the shadow account for employee `system_admin` — re-check that id before reusing it on a different database | **currently applied** |
| [`revert-phase-chat.sql`](./revert-phase-chat.sql) | Undoes that seed exactly, including any message sent through the dashboard while verifying (it deletes by `conversation_id`, not by the seeded id range) | run whenever you want the chat tables empty again |
