...
/**
 * The nine `ComplaintType` enum cases, in the order `StaffComplaintController`
 * declares them. `type` outside this list is rejected with 422.
 *
 * `no_show` is never submitted by a user — `NoshowService::handleConflict()`
 * auto-creates it when both the driver and the passenger press their
 * no-show button within the 2-hour dispute window (see `NoshowService.php`).
 * No automatic penalty is applied for that case, so these rows are the only
 * record of the conflict until an agent resolves them here.
 */
...
/** `ComplaintType::isAutoGenerated()` on the backend — currently true only for `no_show`. */
export const AUTO_GENERATED_COMPLAINT_TYPES: readonly ComplaintType[] = ['no_show'];
