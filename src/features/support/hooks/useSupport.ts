...
import {
  ...
  ESCALATED_STATUSES,
  AUTO_GENERATED_COMPLAINT_TYPES,
} from '../api/supportApi';
import type {
...
export interface Complaint {
  ...
  category: string;
  /**
   * Mirrors `ComplaintType::isAutoGenerated()` on the backend, which the API
   * response does not expose directly. Currently true only for `no_show`
   * conflicts auto-created by `NoshowService::handleConflict()` when both
   * parties press no-show within the 2-hour window — no automatic penalty is
   * applied, so an agent must investigate manually.
   */
...
const mapComplaint = (c: ComplaintResponse, t: TFunction, language: string): Complaint => ({
  ...
  isAutoGenerated: c.type
    ? (AUTO_GENERATED_COMPLAINT_TYPES as readonly string[]).includes(c.type)
    : false,
  title: c.title || '',
